...
def search_and_install(query):
    ...
    try:
        result = run_command(["winget", "search", query])
        if not result.stdout:
            print(Fore.RED + "No output from winget. Check your internet connection or winget installation.")
            return
...
def remove_app(query):
    ...
    try:
        result = run_command(["winget", "list"])
        if not result.stdout:
            print(Fore.RED + "No output from winget. Check your installation." + Style.RESET_ALL)
            return
